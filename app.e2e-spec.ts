  ...
  describe("Quotes (E2E)", () => {
    beforeEach(async () => {
      ...
      const registerResponse = await request(app.getHttpServer())
        .post("/api/v1/auth/login")
        .send({ email: "quote@example.com", username: "quoteuser", password: "password123" });
    ...
    describe("/quotes (POST)", () => {
      it("should create a quote successfully with authentication", async () => {
        const quoteData = { content: "Life is beautiful", author: "Anonymous", category: "Life", tags: ["inspiration", "life"] };
        ...
        expect(response.body.category).toBe(quoteData.category);
        expect(response.body.tags).toEqual(quoteData.tags);
      ...
      it("should fail to create quote with invalid data", async () => {
        const invalidQuoteData = {
          content: "", // Empty content
          tags: "not-an-array" // Should be array
        };

        await request(app.getHttpServer()).post("/api/v1/quotes").set("Authorization", `Bearer ${userToken}`).send(invalidQuoteData).expect(400);
      });
    });
    ...
    describe("/quotes (GET)", () => {
      beforeEach(async () => {
        ...
        for (let i = 0; i < 5; i++) {
          await request(app.getHttpServer())
            ...
            .set("Authorization", `Bearer ${userToken}`)
            .send({
              content: `Test quote ${i + 1}`,
              author: `Author ${i + 1}`,
              category: i % 2 === 0 ? "Life" : "Wisdom",
              tags: [`tag${i + 1}`, "common"]
            });
        }
      });
      ...
      it("should get all quotes with pagination", async () => {
        const response = await request(app.getHttpServer()).get("/api/v1/quotes?page=1&limit=3").expect(200);
      ...
      it("should search quotes by content", async () => {
        const response = await request(app.getHttpServer()).get("/api/v1/quotes?search=quote 1").expect(200);
      ...
      it("should filter quotes by category", async () => {
        const response = await request(app.getHttpServer()).get("/api/v1/quotes?category=Life").expect(200);

        expect(response.body.quotes).toHaveLength(3); // 3 quotes with Life category
      ...
      it("should filter quotes by tags", async () => {
        const response = await request(app.getHttpServer()).get("/api/v1/quotes?tags=tag1").expect(200);

        expect(response.body.quotes).toHaveLength(1);
        expect(response.body.quotes[0].tags).toContain("tag1");
      });

      it("should sort quotes by totalVotes", async () => {
        const response = await request(app.getHttpServer()).get("/api/v1/quotes?sortBy=totalVotes&sortOrder=desc").expect(200);
    ...
    describe("/quotes/:id (GET)", () => {
      ...
      it("should get a single quote by ID", async () => {
        const response = await request(app.getHttpServer()).get(`/api/v1/quotes/${testQuote.id}`).expect(200);
      ...
      it("should return 404 for non-existent quote", async () => {
        ...
        await request(app.getHttpServer()).get(`/api/v1/quotes/${fakeId}`).expect(404);
      });
    });
    ...
    describe("/quotes/:id (PUT)", () => {
      ...
      it("should update quote successfully when no votes exist", async () => {
        ...
        const response = await request(app.getHttpServer())
          .put(`/api/v1/quotes/${testQuote.id}`)
          .set("Authorization", `Bearer ${userToken}`)
          .send(updateData)
          .expect(200);
      ...
      it("should fail to update quote without authentication", async () => {
        await request(app.getHttpServer()).put(`/api/v1/quotes/${testQuote.id}`).send({ content: "Updated content" }).expect(401);
      });
